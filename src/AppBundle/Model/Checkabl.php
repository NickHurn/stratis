...
class Checkabl
{
	...
	private function getJobCheckStatus($user_id, $job_code)
	{
    ...
    public function getBasicData($id)
    {
		...
		$ret2 = $this->execute("select firstname as firstName, surname, mobiletel as mobileNumber, emailaddress as emailAddress from users where employer_id=$id");
		$result = array_merge($ret1[0], $ret2[0]);
		return $result;
	}
    ...
    public function saveWhitelabelData($id,$data)
    {
		...
		{
			copy($data['tmp_name'], $this->logopath."{$domain}_logo.png");
			copy($data['tmp_name'], $this->logopath_zend."{$domain}_logo.png");
